...
export default class ELONavigator extends ContentNavigator implements CardGenerator {
  ...
  async getWeightedCards(limit: number, context?: GeneratorContext): Promise<GeneratorResult> {
    const tElo0 = performance.now(); // [perf] re-enabled 2026-07 (todo-replan-db-perf)
    ...
    const tUser = performance.now(); // [perf] re-enabled 2026-07

    const activeCards = await this.user.getActiveCards();
    const tActive = performance.now(); // [perf] re-enabled 2026-07
    const newCards = (
    ...
    const tCentered = performance.now(); // [perf] re-enabled 2026-07
    logger.info(
