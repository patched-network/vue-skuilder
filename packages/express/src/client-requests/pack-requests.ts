...
export async function packCourse(data: PackCourseData): Promise<PackCourseResponse> {
  ...
  try {
    ...
    // Use packCoursePackage which includes skuilder.json generation
    const packResult = await packer.packCoursePackage(courseDb, originalCourseId, outputPath, fsAdapter);
    ...
    const response: PackCourseResponse = {
      ...
      totalFiles: packResult.filesWritten,
      duration: duration
    };
