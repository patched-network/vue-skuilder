...
describe('Scheduled Review Removal Regression Tests', () => {
  ...
  describe('removeScheduledCardReview bug reproduction', () => {
    it('should actually remove scheduled review from database', async () => {
      ...
      // THIS IS THE BUG: Remove review using the BROKEN calling pattern from StudySession.vue
      ...
      const reviewRemoved = await rawCouch.assertReviewRemoved(testUser.username, scheduledReview._id);
    ...
    it('should handle multiple scheduled reviews correctly', async () => {
      ...
      await user.removeScheduledCardReview(testUser.username, reviewToRemove._id);
      ...
      const reviewRemoved = await rawCouch.assertReviewRemoved(testUser.username, reviewToRemove._id);
      expect(reviewRemoved).toBe(true);
    ...
    it('should handle non-existent review removal gracefully', async () => {
      ...
      await expect(
        user.removeScheduledCardReview(testUser.username, nonExistentReviewId)
      ).rejects.toThrow();
    });
  ...
  describe('StudySession.vue integration patterns', () => {
    it('reproduces exact StudySession.vue calling pattern', async () => {
      ...
      await user.removeScheduledCardReview(user.getUsername(), item._id);
